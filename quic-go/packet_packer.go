...
func (p *packetPacker) PackPing(pf *wire.PingFrame, pth *path, s *session) (*packedPacket, error) {
...
func (p *packetPacker) PackPacket(pth *path, s *session) (*packedPacket, error) {
	...
	if isPing {
		...
		payloadFrames, err = p.composeNextPacket(maxSize, p.canSendData(encLevel), pth, s)
		if err != nil {
			return nil, err
		}
	}
...
func (p *packetPacker) composeNextPacket(
	...
	// TODO: Simplify
	for _, f := range fs {
		f.StreamNumber = s.streamsNumberMap[f.StreamID] + 1
		s.streamsNumberMap[f.StreamID] = f.StreamNumber
		payloadFrames = append(payloadFrames, f)
		// fmt.Println("StreamInfo: ", f.StreamID, f.StreamNumber)
		// if s.scheduler.current_StreamID != uint32(f.StreamID) {
		// 	s.scheduler.current_StreamID = uint32(f.StreamID)
		// 	s.scheduler.flag_train = true
		// }
		...
		// 		defer file.Close()
		// 		fmt.Fprintf(file, "Sending for stream %d of connection %x\n", frame.StreamID, s.connectionID)
		// 	}
		// }
		// if _, err := os.Stat("./logs/server-detail.logs"); err == nil {
		// 	file, _ := os.OpenFile("./logs/server-detail.logs", os.O_APPEND|os.O_WRONLY, 0644)
		// 	defer file.Close()
		// 	fmt.Fprintf(file, "Sending for stream %d\n", f.StreamID)
		// }
	}
