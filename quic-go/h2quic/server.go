...
func (s *Server) handleRequest(session streamCreator, headerStream quic.Stream, headerStreamMutex *sync.Mutex, hpackDecoder *hpack.Decoder, h2framer *http2.Framer) error {
	...
	// Mở file để ghi, với quyền ghi và tạo mới nếu file chưa tồn tại
	file, err := os.OpenFile("./logs/server-detail.logs", os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
	if err != nil {
		panic(err)
	}
	defer file.Close()
	fmt.Fprintf(file, "%s %s%s - ", req.Method, req.Host, req.RequestURI)
	fmt.Fprintf(file, "detail: stream %x with stream-parent %x and weight %d\n", dataStream.StreamID(), protocol.StreamID(h2headersFrame.Priority.StreamDep), h2headersFrame.Priority.Weight+1)
