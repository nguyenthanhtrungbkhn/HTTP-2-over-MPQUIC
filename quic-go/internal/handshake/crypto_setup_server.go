...
func (h *cryptoSetupServer) handleCHLO(sni string, data []byte, cryptoData map[Tag][]byte) ([]byte, error) {
	...
	// if value, ok := cryptoData[TagSFCW]; ok {
	// 	sendStreamFlowControlWindow, _ := utils.LittleEndian.ReadUint32(bytes.NewBuffer(value))
	// 	fmt.Println("PrintSend8: ", sendStreamFlowControlWindow)
	// }
