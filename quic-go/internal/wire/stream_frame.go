...
func ParseStreamFrame(r *bytes.Reader, version protocol.VersionNumber) (*StreamFrame, error) {
	...
	streamNumber, err := utils.GetByteOrder(version).ReadUint16(r)
	if err != nil {
		return nil, err
	}
	frame.StreamNumber = streamNumber
...
func (f *StreamFrame) Write(b *bytes.Buffer, version protocol.VersionNumber) error {
	...
	utils.GetByteOrder(version).WriteUint16(b, f.StreamNumber)
...
func (f *StreamFrame) MinLength(protocol.VersionNumber) (protocol.ByteCount, error) {
	length := protocol.ByteCount(1) + protocol.ByteCount(f.calculateStreamIDLength()) + 2 + f.getOffsetLength() // +2 for StreamNumber
	if f.DataLenPresent {
		length += 2
	}
	return length, nil
