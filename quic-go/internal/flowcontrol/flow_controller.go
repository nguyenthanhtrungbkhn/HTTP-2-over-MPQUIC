...
func (c *flowController) getSendWindow() protocol.ByteCount {
	if c.sendWindow == 0 {
		if c.streamID == 0 {
			// fmt.Println("PrintSend3: ", c.connectionParameters.GetSendConnectionFlowControlWindow())
			return c.connectionParameters.GetSendConnectionFlowControlWindow()
		}
		// fmt.Println("PrintSend3: ", c.connectionParameters.GetSendStreamFlowControlWindow())
		return c.connectionParameters.GetSendStreamFlowControlWindow()
	}
	// fmt.Println("PrintSend3: ", c.sendWindow)
	return c.sendWindow
}

func (c *flowController) AddBytesSent(n protocol.ByteCount) {
	c.bytesSent += n
	// fmt.Println("CheckbyesSent: ", c.bytesSent, n)
}
...
func (c *flowController) SendWindowSize() protocol.ByteCount {
	...
	// fmt.Println("PrintSend2: ", sendWindow-c.bytesSent)
