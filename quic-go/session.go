...
type session struct {
	...
	streamsNumberMap  map[protocol.StreamID]uint16
	streamsNumberRecv map[streamRecv]uint8
	csvWriter         *csv.Writer // Add the CSV writer variable
...
func (s *session) setup(
	...
	if s.config.StreamScheduler == "WRR" || s.config.StreamScheduler == "NII" || s.config.StreamScheduler == "FCFS" || s.config.StreamScheduler == "SWRR" {
		s.streamScheduler = newStreamScheduler()
		s.streamScheduler.nameScheduler = s.config.StreamScheduler
	}
	...
	s.streamsNumberMap = make(map[protocol.StreamID]uint16)
	s.streamsNumberRecv = make(map[streamRecv]uint8)
	file, _ := os.OpenFile("./logs/data-ofo.csv", os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
	s.csvWriter = csv.NewWriter(file)
	// fmt.Println("CreateSess")
...
func (s *session) handleStreamFrame(frame *wire.StreamFrame) error {
	...
	if frame.FinBit {
		...
		if s.perspective == protocol.PerspectiveClient && frame.StreamID > 11 {
			number := s.streamsNumberMap[frame.StreamID]
			record := []string{strconv.FormatUint(uint64(s.connectionID), 10), strconv.FormatUint(uint64(number), 10)}
	...
	//Check Out-of-order OFO Out of order
	if s.perspective == protocol.PerspectiveClient {
		s.streamsNumberRecv[streamRecv{frame.StreamID, frame.StreamNumber}] = 1
		// fmt.Println("Check: ", frame.StreamID, frame.StreamNumber, s.streamsNumberRecv[streamRecv{frame.StreamID, frame.StreamNumber}])
		if frame.StreamNumber > 1 && frame.StreamID > 11 {
			if value, ok := s.streamsNumberRecv[streamRecv{frame.StreamID, frame.StreamNumber - 1}]; ok {
				if value != 1 {
					// fmt.Println("Out-of-order ", value)
					s.streamsNumberMap[frame.StreamID] += 1
				}
			} else {
...
func (s *session) handleAckFrame(frame *wire.AckFrame) error {
	...
	if s.perspective == protocol.PerspectiveServer {
		if err == nil && pth.pathID != protocol.InitialPathID && (s.scheduler.SchedulerName == "FQSAT") {
			if oldLost == newLost {
				s.scheduler.GetStateAndRewardFQSAT(s, pth)
			} else {
				s.scheduler.GetStateAndRewardFQSATLost(s, pth)
			}
		}
...
func (s *session) sendPing(pth *path) error {
	packet, err := s.packer.PackPing(&wire.PingFrame{}, pth, s)
	if err != nil {
		return err
	}
	if packet == nil {
