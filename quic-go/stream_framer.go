...
func (f *streamFramer) maybePopNormalFrames(maxBytes protocol.ByteCount, pth *path) (res []*wire.StreamFrame) {
	...
	if f.streamScheduler.toSend == nil {
		...
		s = f.streamScheduler.toSend //never into case
		// fmt.Println("Check2")
	}
	if s != nil && s.stream != nil {
		if s.stream.streamID != 3 {
			if _, err := os.Stat("./logs/server-detail.logs"); err == nil {
				file, _ := os.OpenFile("./logs/server-detail.logs", os.O_APPEND|os.O_WRONLY, 0644)
				defer file.Close()
				fmt.Fprintf(file, "Sending for stream %d of weight %d quantum %d\n", s.stream.streamID, s.weight+1, s.quantum)
			}
		}
		// if s.quantum == 0 {
		// 	pth.sess.scheduler.getActionAsync(baseURL+"/get_action", pth.sess.paths)
		// } else if s.quantum%10 == 0 {
		// 	pth.sess.scheduler.getActionAsync(baseURL+"/get_action", pth.sess.paths)
		// }
